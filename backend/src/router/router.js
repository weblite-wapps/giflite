...
router.post("/search", (req, res) => {
  ...
  request(
    `http://api.giphy.com/v1/gifs/search?q=${
      req.body.info
    }&api_key=mX3Dx22ZGrswOXaCUw1tVVM23Jn3atiz&limit=3`,
    (error, response, body) => {
      const parsedBody = JSON.parse(body)
      // console.log(parsedBody.data[1].images) // Print the error if one occurred
      ...
      const urls = paresdBodyData.map(x =>
        R.prop("url", R.prop("downsized", R.prop("images", x))),
      )
